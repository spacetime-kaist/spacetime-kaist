...
export default function LandingPageV4() {
  ...
  const resolvedRelated = useMemo(() => {
    ...
    const resolve = ({ data, id }) => {
      ...
      if (!item) return null;
      if (data === 'press')         return { type: 'press', subtype:'press', label: 'Press',       date: item.date,  title: item.title, href: item.link_en || item.link_kr };
      if (data === 'outreach')      return { type: 'event', subtype:item.category, label: item.category || 'Event', date: item.start, title: item.title, href: `/events?scroll=${item.id}` };
      if (data === 'lablife')       return { type: 'event', subtype:item.category, label: 'Lab Life',    date: item.start, title: item.title, href: `/events?scroll=${item.id}` };
      if (data === 'publications')  return { type: 'pub',   subtype:'journal', label: 'Journal',     date: item.date,  title: item.title, href: item.href };
      if (data === 'international') return { type: 'pub',   subtype:'conf', label: 'Intl. Conf.', date: item.date,  title: item.title, href: item.href };
      if (data === 'national')      return { type: 'pub',   subtype:'conf', label: 'Natl. Conf.', date: item.date,  title: item.title, href: item.href };
      return null;
    };
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-10 lg:gap-14">
                    ...
                    <div className="md:col-span-1">
                      ...
                      {/* <p className="mt-5 text-[10px] text-gray-400">
                        {recentPubs.length} total publications &nbsp;·&nbsp;{' '}
